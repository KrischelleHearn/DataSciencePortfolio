...
```{r}
july_2020 = filter(cpi, year==2020, month_name =='jul')

housing = mutate(housing,
                 cpi_latest = july_2020$cpi,
                 multiplier = cpi_latest/cpi,
                 volume_adj = volume*multiplier,
                 median_adj = median*multiplier)

housing %>% 
select(c("city", "year","month", "volume", "median", "cpi_latest", "multiplier", "volume_adj", "median_adj")) %>% head()
```
...
housing_1 %>% 
select(c("city", "year", "month", "volume_adj_max", "volume_adj_min", "median_adj_max", "median_adj_min")) %>% 
head()
...
housing_max %>% 
select(c("city", "year", "month", "median_adj", "median_adj_max", "median_adj_min")) %>% 
head()
```
...
housing_sum2 %>% 
select(c("year", "month", "listings_med", "sales_med")) %>% 
head()
...
housing_2 %>% 
select(c("city", "year", "month", "listings", "sales", "listings_med", "sales_med")) %>% 
head()
```
...
housing_2 = mutate(housing_2,
                 ...
                 sales_ind = ifelse(housing_2$sales <= sales_med, 0, 1))

housing_2 %>% 
select(c("city", "year","month", "listings", "listings_med", "listings_ind", "sales", "sales_med", "sales_ind")) %>% head()
...
housing_2 = mutate(housing_2,
                   market_hotness = case_when(listings_ind < 1 | sales_ind < 1 ~ "Low",
                                            listings_ind < 1 | sales_ind > 0 ~ "High",                   
                                            listings_ind > 0 | sales_ind < 1 ~ "Very_Low",
                                            listings_ind > 0 | sales_ind > 0 ~ "Average"))

housing_2 %>% 
select(c("city", "year", "month", "listings", "sales", "listings_ind", "sales_ind", "market_hotness")) %>%  
head()
